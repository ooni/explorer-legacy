...
function automigrateTables(cb) {
    ds.automigrate(lbTables, function(err) {
        if (err) cb(err);
        console.log('Looback tables [' + lbTables + '] created in ',
                    ds.adapter.name);
        cb();
    });
}
...
function createCountryRows(cb) {
  ...
  ds.models.country.create(country_rows, cb);
}

function createNettestRows(cb) {
    if (err) { console.log(err) }
    console.log('Inserting nettest data');
    ds.models.nettest.create(nettest_rows, cb);
});
